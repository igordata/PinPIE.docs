...
<article>
  ...
  <p>
    ...
    сжатие gzip, и поддержка серверного шардинга. Статик теги начинаются с символа <b>%</b> и типа статик контента,
...
//pinpie.ru/images/avatar.png?time=b17c93f172020c8c4c0f324b048b3434', 'html') ?>
  ...
  <section>
    ...
    <p>
      Современные браузеры могут скачивать контент страницы с одного сайта примерно в пять одновременных потоков.
    ...
    <p>
      Если по каким-то прчинам вы не можете этого сделать, вам придётся увеличисть количество одновременных потоков
    ...
    <p>
      ...
      <?= scx('CFG::$static_servers', 'php') ?>. По умолчанию это пустой массив, т.е. шардинг не осуществляется.
    </p>
    <p>
      Сервер выбирается в методе <?= scx('Staticon::getServer($file)', 'php') ?>, где $file это путь к файлу внутри
      <?= scx('CFG::$pinpie["static folder"]') ?>. На текущий момент нет способа переопределить этот метод в конфиге,
  ...
  <section>
    ...
    <p>
      ...
      Тип img не допукает ни того, ни другого. Типы, которые можно минифицировать или сжимать содержатся в массивах
      <?= scx('CFG::$pinpie["static minify types"]', 'php') ?> и <?= scx('CFG::$pinpie["static gzip types"]', 'php') ?>
    ...
    <p>
      ...
      <?= scx('CFG::$pinpie["static gzip level"]', 'php') ?>. По умолчанию это пять.
    </p>
  </section>
  ...
  <section>
    ...
    <p>
      ...
      Вы можете установить уровень сжатия в <?= scx('CFG::$pinpie["static gzip level"]', 'php') ?>.
    </p>
    ...
    <?= pcx("['js', 'css']") ?>
    <p>
      Если тип есть в массиве, тогда файл "имяфайла.gz" будет создан. Если веб-сервер правильно настроен, то он станет использовать
  ...
  <section>
    ...
    <p>
      ...
      минификацию, стоит лишь вносить путь в некий список запланированных минификаций, и по крону минифицировать.
    </p>
...
$pinpie["static minify function"] = function(){/.../};') ?>
  ...
  return true;
    ...
    <ul>
      <li>
        ...
        Его значение <?= scx('CFG::$pinpie["static folder"] . "/main.css"') ?>.
      </li>
      <li>
        <?= scx('$minfilepath') ?> путь к минифицированной версии, которую нужно проверить.
        Значение: <?= scx('CFG::$pinpie["static folder"] . "/main.min.css"') ?>
      </li>
      <li>
      ...
      <li>
        <?= scx('CFG::$pinpie["static minify types"]') ?> это массив с дефолтным значением <?= scx('["js", "css"]') ?>.
  ...
  <section>
    ...
    <?= pcx('$pinpie["static minify types"] = [];     // отключает минификацию
$pinpie["static gzip types"] = [];       // отключает сжатие gzip
$pinpie["static dimensions types"] = []; // отключает сбор картинковых размеров', 'php') ?>
...
<div class="wrapper">//s2.somesite.com/jpg.jpg?time=30c1c523bed6d902df19d9637831e41c</div>', 'html')) ?>
  ...
  ">
    ...
    <p>Помните: чтобы просто получить путь, можете просто проставить восклицательный знак.</p>
